...
sub save_array_as_json {
	my ($filename, @array) = @_;
	open my $file, '>:encoding(utf8)', "databases-json/$filename" or die "Cannot open morph.json";
	try {
		my $json = JSON::MaybeXS->new->encode(\@array);
		print $file $json;
	} catch {
		print("CAUGHT $_");
	};
...
sub new {
	...
	croak $errormsg if ($@ or !$ref);
	@MORPH = @$ref;
	save_array_as_json("morph.json", @MORPH);
...
sub clean_lookup
{
	...
	for my $href ( @FOCAIL ) {
		if ( exists($href->{$current}) ) {
			my $ret = $self->get_grammar_tags($href->{$current});
			return $ret;
		}
	}
	return "";
...
sub add_tags_real
{
	...
	open unchecked_xml_fh, '>>:encoding(UTF-8)', "test-ref/unchecked_xml.line-json";
	my %unchecked_xml_io;
 	$unchecked_xml_io{"input"} = $sentence;
	$sentence = unchecked_xml($sentence);
	$unchecked_xml_io{"output"} = $sentence;
	$unchecked_xml_io{"same"} = ($unchecked_xml_io{"input"} eq $unchecked_xml_io{"output"}) ? "true" : "false";
	print unchecked_xml_fh JSON::MaybeXS->new->encode(\%unchecked_xml_io), "\n";

	open comhshuite_fh, '>>:encoding(UTF-8)', "test-ref/comhshuite.line-json";
	my %comhshuite_io;
 	$comhshuite_io{"input"} = $sentence;
	comhshuite($sentence);
	$comhshuite_io{"output"} = $sentence;
	$comhshuite_io{"same"} = ($comhshuite_io{"input"} eq $comhshuite_io{"output"}) ? "true" : "false";
	print comhshuite_fh JSON::MaybeXS->new->encode(\%comhshuite_io), "\n";

	open aonchiall_fh, '>>:encoding(UTF-8)', "test-ref/aonchiall.line-json";
	my %aonchiall_io;
 	$aonchiall_io{"input"} = $sentence;
	aonchiall($sentence);
	$aonchiall_io{"output"} = $sentence;
	$aonchiall_io{"same"} = ($aonchiall_io{"input"} eq $aonchiall_io{"output"}) ? "true" : "false";
	print aonchiall_fh JSON::MaybeXS->new->encode(\%aonchiall_io), "\n";

	open aonchiall_deux_fh, '>>:encoding(UTF-8)', "test-ref/aonchiall_deux.line-json";
	my %aonchiall_deux_io;
 	$aonchiall_deux_io{"input"} = $sentence;
	aonchiall_deux($sentence);
	$aonchiall_deux_io{"output"} = $sentence;
	$aonchiall_deux_io{"same"} = ($aonchiall_deux_io{"input"} eq $aonchiall_deux_io{"output"}) ? "true" : "false";
	print aonchiall_deux_fh JSON::MaybeXS->new->encode(\%aonchiall_deux_io), "\n";

	open unigram_fh, '>>:encoding(UTF-8)', "test-ref/unigram.line-json";
	my %unigram_io;
 	$unigram_io{"input"} = $sentence;
	unigram($sentence);
	$unigram_io{"output"} = $sentence;
	$unigram_io{"same"} = ($unigram_io{"input"} eq $unigram_io{"output"}) ? "true" : "false";
	print unigram_fh JSON::MaybeXS->new->encode(\%unigram_io), "\n";
...
sub grammatical_errors
{
  ...
  # endoflast is global offset in $pristine following the end of last error
	...
	my @add_tags_real_io_array = [];
	my $add_tags_real_fh;
	open add_tags_real_fh, '>:encoding(UTF-8)', 'test-ref/add_tags_real.line-json';
	foreach my $plain (@$sentences) {
		my %add_tags_real_io;
		$add_tags_real_io{"input"} = $plain;
		my $s = $self->add_tags_real($plain);
		$add_tags_real_io{"output"} = $s;
		print add_tags_real_fh JSON::MaybeXS->new->encode(\%add_tags_real_io), "\n";
...
sub strip_errors
{
	my $strip_errors_file;
	open $strip_errors_file, '>>', 'test-ref/strip_errors.json';
	my %strip_errors_io;
	$strip_errors_io{"input"} = @_;
	...
	$strip_errors_io{"output"} = @_;
	my $json = JSON::MaybeXS->new->encode(\%strip_errors_io);
	print $strip_errors_file "$json\n";
	print($json);
}
...
{
	my $rialacha_file;
	open $rialacha_file, '>>', 'test-ref/rialacha.json';
	my %rialacha_io;
	$rialacha_io{"input"} = $_[0];
	for ($_[0]) {
	...
	$rialacha_io{"output"} = $_[0];
	my $json = JSON::MaybeXS->new->encode(\%rialacha_io);
	print $rialacha_file $json, "\n";
	print($json);
}
